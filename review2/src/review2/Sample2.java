...
public class Sample2 {
	...
	public static void main(String[] args) {
		...
		// 배열을 이용하여 투구수를 저장할 수 있을까? 
		// 1회에 투구수가 3개도 될 수 있고 100개도 될 수 있지 않은가? 
		...
		// 인텔리제이에서 위의 예제와 같이 제네릭스 없이 코딩하면 명확한 타입을 명시하라는 warning이 표시될 것이다. 
		//이 부분에 대한 자세한 내용은 잠시 후에 자세하게 설명한다.
		...
		// ※ JSSE 5.0 이후 버전에서 두번째 방법으로 코드를 작성해도 오류는 나지 않는다. 
		// 다만 제네릭스 타입을 명확하게 지정하라는 Warning 메시지가 출력된다.
		...
		String one = (String)aList.get(0);// Object 자료형을 String 자료형으로 캐스팅한다.
	    String two = (String)aList.get(1);
	    
	    // 위처럼 제네릭스를 사용하지 않을 경우에는 ArrayList 안에 추가되는 객체는 Object 자료형으로 인식된다. 
	    // Object 자료형은 모든 객체가 상속하고 있는 가장 기본적인 자료형이다. 
	    // 따라서 ArrayList 객체인 aList에 값을 넣을 때는 문제가 안되지만 
	    // 값을 가져올 경우에는 항상 Object 자료형에서 String 자료형으로 다음과 같이 형변환(casting)을 해 주어야만 한다.
	    
	    // 또 한가지 주의할 점은 aList 안에는
	    // String 객체 이외의 객체도 넣을 수 있기 때문에 형 변환 과정에서 잘못된 형변환으로 인한 오류가 발생할 가능성이 있다는 점이다. 
	    ...
	    String one2 = aList2.get(0);// 형 변환이 필요없다.
	    String tow2 = aList2.get(1);// 형 변환이 필요없다.
	    
	    // 제네릭스로 자료형을 선언하기만 하면 그 이후로는 자료형에 대한 형변환 과정이 필요없다. 
	    // 이미 컴파일러가 aList2에는 반드시 String 자료형만 추가 되어야 함을 알기 때문이다. 
	    ...
	    pitches3.add("115");
	    pitches3.add("149");
	    System.out.println(pitches3);// [133, 115, 149] 출력
	    
	    // 하지만 다음처럼 이미 데이터가 존재할 경우에는 보다 편하게 ArrayList를 생성할 수 있다.
	    String[] data = {"138","144","150"};// 이미 투구수 데이터 배열이 있더.
	    ArrayList<String> pitches4 = new ArrayList<>(Arrays.asList(data));
	    System.out.println(pitches4);// [138, 144, 150]출력
	    ...
	    ArrayList<String> pitches5 = new ArrayList<>(Arrays.asList("133","150","170"));
	...
	pitches9.sort(Comparator.naturalOrder());// 오름차순으로 정렬
	System.out.println(pitches9);// [129, 138, 142] 출력
